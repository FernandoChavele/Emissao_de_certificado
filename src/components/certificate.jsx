...
const numberToWords = (num) => {
    ...
    if (num >= 0 && num <= 20) {
        ...
        return num.toString(); // Para números fora do intervalo, retorna o valor como string
    }
};
...
const Certificate = ({ studentName, biNumber, directorName, courseName, startDate, endDate, grade }) => {
    ...
    const generatePDF = () => {
        ...
        html2canvas(input, { scale: 2 }).then((canvas) => {
            ...
            button.style.display = 'block';  // Reexibe o botão após salvar
        });
    };

    const currentDate = new Date().toLocaleDateString();  // Data de emissão dinâmica
    const gradeInWords = numberToWords(parseInt(grade));  // Converte a nota para extenso
